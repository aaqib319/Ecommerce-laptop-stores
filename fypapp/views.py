...
def checkout_view(request):
    """
    Handle the checkout view where users' order details are sent to Google Sheets.
    """
    cart_items = CartItem.objects.filter(user=request.user)
    ...
    total_price = sum(item.attribute.price for item in cart_items if item.attribute)
    shipping_cost = 2000
    final_total = total_price + shipping_cost  # No conversion to paisa needed now
    ...
    if request.method == 'POST':
        ...
        if send_to_google_sheet({
            ...
            "cart_items": [{'product': item.product.name, 'quantity': item.quantity} for item in cart_items]
        }):
            messages.success(request, "Your order has been placed successfully, and your details have been sent to Google Sheets.")
        else:
            messages.error(request, "Failed to send your details to Google Sheets.")
        
        # Redirect to order confirmation or another page after sending details
        return redirect("order_confirmation")  # Replace with your desired URL for confirmation

    return render(request, "checkout.html", {"cart_items": cart_items, "total_price": total_price, "shipping_cost": shipping_cost, "final_total": final_total})
